...
# ====== 1. 读取并预处理原始数据 ======
path = r'C:/Users/Administrator/Desktop/清除数据的数据库/alloy_database_clean_YS1.xlsx'  # 换成你的实际路径
...
# 列名
element_cols = [c for c in df.columns if c not in ('Coded','YS(MPa)')]

# 拆分数据
X_elems   = df[element_cols].values.astype(np.float32)
coded_raw = df[['Coded']].values.astype(np.float32)
y_raw     = df['YS(MPa)'].values.reshape(-1,1).astype(np.float32)
...
class Generator(nn.Module):
    ...
    def forward(self, z):
        ...
        feat = self.fc3(h)
        ys   = self.fc4(z)
        return torch.cat([feat, ys], dim=1)
...
yssynth = synth[:, feat_dim]
Xsynth = scaler_X.inverse_transform(Xsynth)
yssynth = scaler_y.inverse_transform(yssynth.reshape(-1,1)).flatten()
Xsynth = np.maximum(Xsynth, 0.0)
yssynth = np.maximum(yssynth, 0.0)
...
sum_oth  = Xsynth[:, idx_oth].sum(axis=1)
sum_oth[sum_oth==0] = 1.0
scale    = (100 - M) / sum_oth
Xsynth[:, idx_oth] = (Xsynth[:, idx_oth].T * scale).T

# YS 截断（建议取原始数据库的极值，如90, 650）
ys_min, ys_max = float(df['YS(MPa)'].min()), float(df['YS(MPa)'].max())
yssynth = np.clip(yssynth, ys_min, ys_max)

# 合并 DataFrame
df_gen = pd.DataFrame(
    np.hstack([Xsynth, yssynth.reshape(-1,1)]),
    columns=element_cols + ['Coded','YS(MPa)']
)

# ====== 5. 保存 ======
out_dir  = r'C:/Users/Administrator/Desktop/cleans'
os.makedirs(out_dir, exist_ok=True)
out_path = os.path.join(out_dir, 'alloy_augmented_WGAN_GP_YS.xlsx')
