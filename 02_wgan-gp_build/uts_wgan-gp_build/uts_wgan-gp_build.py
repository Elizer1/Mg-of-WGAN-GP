...
# ====== 1. 读取并预处理原始数据 ======
path = r'C:/Users/Administrator/Desktop/清除数据的数据库/alloy_database_clean_UTS2.xlsx'
...
# 列名
element_cols = [c for c in df.columns if c not in ('Coded','UTS(MPa)')]

# 拆分数据
X_elems   = df[element_cols].values.astype(np.float32)
coded_raw = df[['Coded']].values.astype(np.float32)
y_raw     = df['UTS(MPa)'].values.reshape(-1,1).astype(np.float32)
...
class Generator(nn.Module):
    ...
    def forward(self, z):
        ...
        feat = self.fc3(h)
        uts  = self.fc4(z)
        return torch.cat([feat, uts], dim=1)
...
utsynth = synth[:, feat_dim]
Xsynth = scaler_X.inverse_transform(Xsynth)
utsynth = scaler_y.inverse_transform(utsynth.reshape(-1,1)).flatten()
Xsynth = np.maximum(Xsynth, 0.0)
utsynth = np.maximum(utsynth, 0.0)
...
# UTS 截断
utsynth = np.clip(utsynth, 90, 650)

# 合并 DataFrame
df_gen = pd.DataFrame(
    np.hstack([Xsynth, utsynth.reshape(-1,1)]),
    columns=element_cols + ['Coded','UTS(MPa)']
)

# ====== 5. 保存 ======
out_dir  = r'C:/Users/Administrator/Desktop/cleans'
os.makedirs(out_dir, exist_ok=True)
out_path = os.path.join(out_dir, 'alloy_augmented_WGAN_GP_UTS.xlsx')
